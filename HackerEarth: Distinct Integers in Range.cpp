...
int A[N], B[N];
bitset <5050> tree[N*4], tree2[N*4];
...
bitset<5050> query1(int node, int b, int e, int l, int r){
    ...
    return query1(node*2, b, mid(b,e), l, r) | 
           query1(node*2+1, mid(b,e)+1, e, l, r);
}

bitset<5050> query2(int node, int b, int e, int l, int r){
    if(b > r or e < l or b > e){
        return 0;
    }
    if(b >= l and e <= r){
    ...
    return query2(node*2, b, mid(b,e), l, r) | 
           query2(node*2+1, mid(b,e)+1, e, l, r);
}
...
int32_t main(){
    ...
    while(tc--){
        ...
        int q; I(q);
        while(q--){
            int a, b, c, d; II(a, b); II(c, d);
            int ans = (query1(1,1,n,a,b) 
                        | query2(1,1,n,c,d)).count();
            P(ans); nl;
        }
    }
    return 0;
